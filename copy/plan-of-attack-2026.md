...
- **Legal pages — drafted, not published (found uncommitted 2026-08-29, drafted 2026-08-25):** `copy/privacy-policy.md`, `copy/terms-and-conditions.md`, `copy/disclaimers.md`. Disclaimers already matches the live `/disclaimers` page. Privacy and Terms aren't live yet — **Privacy is a real blocker**, not just housekeeping: LIPS-CRM's booking-flow Google OAuth verification needs `lipservices.com.au/privacy` live with real content before Google will take it out of Testing mode (see `LIPS-CRM/docs/brief-booking-scarcity-flow.md`). Privacy's draft flags one open item (confirm Resend's per-org plan status before publishing); Terms' draft has no open gaps, just wants a solicitor pass. The old `/disclosures` page (2022 investment-style boilerplate) should be deleted once the new disclaimers content is confirmed live.
